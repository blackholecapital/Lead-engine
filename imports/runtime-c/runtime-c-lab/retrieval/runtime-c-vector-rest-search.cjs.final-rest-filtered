...
(async()=>{
  ...
  const t=await db.openTable("runtime_c_vectors_rest");
  const raw=(await t.search(hashVec(query)).limit(Number(process.env.RAW_K||120)).toArray()).filter(h=>!String(h.path||h.text||"").match(/package\.json$/)).slice(0,Number(process.env.TOP_K||20));
  console.log(JSON.stringify(raw.map(h=>({
    score:Number((100-(h._distance||9)*20).toFixed(3)),
    dist:Number((h._distance||0).toFixed(3)),
    kind:h.kind,key:h.key,repo:h.repo,domain:h.domain,path:h.path,
    text:String(h.text||"")
  })),null,2));
})();
